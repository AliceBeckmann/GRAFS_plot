...
change_bubble_size <- function(txml, label, NEW_SIZE, NEW_TEXT_SIZE) {
  ...
  for (i in c(50.5, 50, 30)) {
    newauxt <- str_replace(
      newauxt,
      paste0('width="', i, '"'),
      paste0('width="', NEW_SIZE, '"')
    )
    newauxt <- str_replace(
      newauxt,
      paste0('height="', i, '"'),
      paste0('height="', NEW_SIZE, '"')
    )
  }

  txml <- str_replace(txml, auxt, newauxt)
...
change_style <- function(txml, id, identifier, value, XML_temp, T_ID_ARROW) {
  ...
  if (!is.na(numcell)) {
    ...
    stact <- xmlAttrs(root[["diagram"]][["mxGraphModel"]][["root"]][[numcell]])[
      "style"
    ]
    aux <- str_split(stact, ";")
    ...
    xmlAttrs(root[["diagram"]][["mxGraphModel"]][["root"]][[numcell]])[
      "style"
    ] <- stnew
...
change <- function(
  ...
  if (!any(is.na(DATOch_flat))) {
    for (ii in 1:length(DATOch)) {
      # print(DATOch[[ii]]$old)
      if (str_to_lower(dato) == str_to_lower(DATOch[[ii]]$old)) {
        dato <- DATOch[[ii]]$new
      }
    }
  }
  ...
  if (length(x) > 0) {
    ...
    if (!is.na(T_ID_ARROW$labelchange[x]) & PLOT_CHANGE) {
      # & !is.na(value_change)){
      # if(!is.na(T_ID_ARROW$labelchange[x])){ # & !is.na(value_change)){

      lbchange <- T_ID_ARROW$labelchange[x]
      if (dato == 0 || is.na(value_change)) {
        # si antes era 0 o el nuevo es 0. Si el nuevo es 0 implica reduccion de -100 pero no hay que ponerlo
        # print(str_replace_all(str_replace_all(lbchange,">",""),"<",""))
        # if(!SIMPLIFIED_PLOT){
        txml <- remove_id(
          txml,
          str_replace_all(str_replace_all(lbchange, ">", ""), "<", "")
        )
        # }
      } else {
    ...
    for (PROV_ACT in REGIONS) {
      ...
      if (PLOT_CHANGE) {
        txml <- change(
          txml,
          "<YEARCHANGE>",
          paste0("(changes_", YEARS_CHANGE[1], "-", max(YEARS_CHANGE), ")"),
